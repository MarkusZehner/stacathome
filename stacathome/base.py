...
class BaseMetadataProcessor(ABC):
    ...
    @classmethod
    @abstractmethod
    def request_items(cls, _, request_time, request_place, max_items=-1, **kwargs) -> list:
    ...
    @abstractmethod
    def get_bbox(self):
        """return the bounding box as shapely.box in the crs provided by get_crs"""
        raise NotImplementedError
    ...
    def get_resampling_per_band(self, target_resolution):
        return {b.name: ("nearest"
                         if target_resolution <= b.spatial_resolution or
                         math.isclose(target_resolution, b.spatial_resolution)
                         else "bilinear"
...
class EarthAccessProcessor(BaseMetadataProcessor):
    ...
    def request_items(cls, _, request_time, request_place, max_items=-1, **kwargs):
        ...
        if max_items != -1 or 'maxResults' in kwargs:
            kwargs['count'] = max(kwargs.get('maxResults', -1), max_items)
...
class ASFResultProcessor(BaseMetadataProcessor):
...
class STACItemProcessor(BaseMetadataProcessor):
    ...
    @classmethod
    def download_tiles_to_file(cls, path, items, bands, _):
        dl_items = []
        for i in items:
            assets = i.get_assets()
        ...
        cls.provider.download_granule(dl_items)

    @classmethod
    def request_items(cls, collection, request_time, request_place, max_items=None, **kwargs):
