...
def smallest_modulo_deviation(r, m):
    """return closest value around zero, either remainder or remainder - divisor"""
    r_mod = r % m
    alt = r_mod - m
...
class ECOL2TLSTEProcessor(EarthAccessProcessor):
    ...
    def download_tiles_to_file(cls, path, items, bands=None, processes=4):
        ...
        cls.provider.download_granule(granules_links_flat, path, threads=processes)
...
class OPERASentinel1RTCProcessor(ASFResultProcessor):
    ...
    def download_tiles_to_file(cls, path, items, bands, processes=4):
        ...
        cls.provider.download_granule(dl_items_flat, path=path, processes=processes)
