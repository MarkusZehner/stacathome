...
for i in range(min([len(geometry_filtered_items), 2])):
    m = geometry_filtered_items[i].geometry_odc_geometry.explore(m, simplify=0, name=f'geo_{i}', tooltip=f'geo_{i}',
        style_function=lambda feature: {
            'fillColor': 'orange',
            'color': 'orange',
            'weight': 2,
...
print(set([g.id for g in geometry_filtered_items]))
m