...
if __name__ == '__main__':
    ...
    m = area_of_interest.explore(
        m,
        simplify=0,
        name='roi',
        tooltip='roi',
        style_function=lambda feature: {'fillColor': 'red', 'color': 'red', 'weight': 2, 'fillOpacity': 0.1},
    )
    ...
    m = roi_small.explore(
        ...
        name='roi_small',
        tooltip='roi_small',
        style_function=lambda feature: {'fillColor': 'blue', 'color': 'blue', 'weight': 2, 'fillOpacity': 0.1},
    )
    ...
    for i in range(4):
        m = only_newer_processing[i].geometry_odc_geometry.explore(
            m,
            simplify=0,
            name=f'new_{i}',
            tooltip=f'new_{i}',
    ...
    for i in range(4):
        m = coverage_filtered_items[i].geometry_odc_geometry.explore(
            m,
            simplify=0,
            name=f'cov_{i}',
            tooltip=f'cov_{i}',
            style_function=lambda feature: {'fillColor': 'black', 'color': 'black', 'weight': 2, 'fillOpacity': 0.1},
        )

    print({g.id for g in s2_items})
    print({g.id for g in only_newer_processing})
    print({g.id for g in coverage_filtered_items})
    m
